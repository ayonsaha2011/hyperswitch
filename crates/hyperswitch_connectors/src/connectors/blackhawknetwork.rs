...
impl
    ...
    fn get_request_body(&self, req: &PaymentsAuthorizeRouterData, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_router_data =
            blackhawknetwork::BlackhawknetworkRouterData::from((
                amount,
                req,
            ));
        let connector_req = blackhawknetwork::BlackhawknetworkPaymentsRequest::try_from(&connector_router_data)?;
    ...
    fn handle_response(
        ...
        let response: blackhawknetwork::BlackhawknetworkPaymentsResponse = res.response.parse_struct("Blackhawknetwork PaymentsAuthorizeResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
...
impl
    ...
    fn get_request_body(&self, req: &RefundsRouterData<Execute>, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_req = blackhawknetwork::BlackhawknetworkRefundRequest::try_from(&connector_router_data)?;
        Ok(RequestContent::Json(Box::new(connector_req)))
    }
    ...
    fn handle_response(
        ...
        let response: blackhawknetwork::RefundResponse = res.response.parse_struct("blackhawknetwork RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
...
impl
    ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Blackhawknetwork {
    ...
    fn handle_response(
        ...
        let response: blackhawknetwork::RefundResponse = res.response.parse_struct("blackhawknetwork RefundSyncResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
